...
INSERT INTO app_config (id, total_tiles, grid_columns) VALUES (1, 4, 2);
...
BEGIN
    ...
    -- Get total tiles from config
    SELECT total_tiles INTO v_total_tiles FROM app_config WHERE id = 1;
    
    -- Calculate total universe value
    v_total_universe_value := calculate_total_tile_cost(v_total_tiles);
    
    -- Update weightages for all unpurchased tiles
    UPDATE tiles 
    SET weightage = (v_current_price / v_total_universe_value) * 100,
        updated_at = TIMEZONE('utc'::text, NOW())
    WHERE is_purchased = FALSE AND id < v_total_tiles;
    
    RAISE NOTICE 'Updated weightages: % purchased, % tiles, current price %, total value %', 
        v_total_purchased, v_total_tiles, v_current_price, v_total_universe_value;
END;
$$ LANGUAGE plpgsql;

-- Update get_total_universe_value to use config
CREATE OR REPLACE FUNCTION get_total_universe_value()
RETURNS DECIMAL(30,2) AS $$
DECLARE
    tile_count INTEGER;
BEGIN
    SELECT total_tiles INTO tile_count FROM app_config WHERE id = 1;
    RETURN calculate_total_tile_cost(tile_count);
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF p_tile_id >= v_total_tiles THEN
        RAISE EXCEPTION 'Tile ID % is beyond configured tile count %', p_tile_id, v_total_tiles;
    END IF;
    ...
    -- Calculate total universe value using config
    v_total_universe_value := calculate_total_tile_cost(v_total_tiles);
    ...
    SET total_purchased = total_purchased + 1,
        current_price = get_fibonacci_price(total_purchased + 1)
    WHERE id = 1
